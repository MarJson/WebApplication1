...
namespace System
{
    ...
    {
        public static Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<Guid, bool>>>> permissionDic =
            new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<Guid, bool>>>>();

        public static bool CheckPermission(Guid roleId, string namespaceName, string controllerName, string actionName)
        {
            if (roleId.Equals(ConfigHelper.SuperRole))
                return true;
            if (permissionDic == null || permissionDic.Count() == 0)
            {
                permissionDic = new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<Guid, bool>>>>();
        ...
        public static void RegisterPermissionDic()
        {
            ...
            foreach (FunctionPoint action in iaction.Entities)
            {
                ...
                {
                    permissionDic[action.Namespace] = new Dictionary<string, Dictionary<string, Dictionary<Guid, bool>>>();
                }
                if (!permissionDic[action.Namespace].ContainsKey(action.Controller))
                {
                    permissionDic[action.Namespace][action.Controller] = new Dictionary<string, Dictionary<Guid, bool>>();
                }
                if (!permissionDic[action.Namespace][action.Controller].ContainsKey(action.Action))
                {
                    permissionDic[action.Namespace][action.Controller][action.Action] = new Dictionary<Guid, bool>();
                }
